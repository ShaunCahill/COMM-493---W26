...
# INSTRUCTION: Replace with your LDA POS SageMaker endpoint name
# from the LDA Topic Modeling POS notebook (STEP 6: Deploy Model)
ENDPOINT_NAME = "your-lda-pos-endpoint-name"
...
# LDA Topic Modeling POS notebook. In your notebook at STEP 4
...
def lambda_handler(event, context):
    """
    AWS Lambda function for LDA topic modeling on POSITIVE reviews.

    Receives raw positive review text from the web dashboard, preprocesses
    and vectorizes each review using the embedded vocabulary, then sends
    the document-term vectors to the LDA POS SageMaker endpoint.
    ...
    # Call the LDA POS SageMaker endpoint
    try:
